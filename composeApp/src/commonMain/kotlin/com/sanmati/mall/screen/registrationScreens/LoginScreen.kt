...
fun LoginUI(
    ...
    Card(
        colors = CardDefaults.cardColors(containerColor = Surface),
        shape = RoundedCornerShape(16.dp),
        modifier = Modifier.padding(16.dp).fillMaxSize().wrapContentHeight()
    ) {
    ...
    Column(
        ...
        FilledTextField(
            ...
            keyBoardActions = KeyboardActions(
                onNext = {
                    focusManager.moveFocus(FocusDirection.Down) // move to next input
                }
            ),
            leadingIcon = {
                Icon(
        ...
        FilledTextField(
            ...
            *//*leadingIcon = { Icon(imageVector = Icons.Rounded.Lock, contentDescription = "password", tint = OnSurfaceVariant) },
            trailingIcon = { val icon = if (passwordVisible) Icons.Rounded.Visibility else Icons.Rounded.VisibilityOff IconButton (onClick = onTogglePassword) { Icon(imageVector = icon, contentDescription = null, tint = PrimaryDark) } },
             visualTransformation = if (passwordVisible) VisualTransformation.None else PasswordVisualTransformation()*//*
        )
        *//*Spacer(modifier = Modifier.height(8.dp))

        TextButton( onClick = { *//**//* TODO: Forgot password *//**//* }, modifier = Modifier.align(Alignment.End) ) { Text("Forgot password?", color = OnSurfaceVariant) }*//*
        ...
        CustomButton(
            ...
            onClick = {
                ...
                scope.launch {
                    ...
                    if (mobileNumber.length != 10 || password.length < 3) {
                        ...
                        viewModel.performLogin(mobileNumber, password)
            ...
            isLoading = isLoading,
            enabled = mobileNumber.isNotBlank() && password.isNotBlank(),
